...
test_that("[chisq-001] calling rvgt.range.chisq", {
        ...
        chisq.q <- rvgt.range.chisq(gen.data = mgt,
...
test_that("[chisq-002] calling rvgt.range.chisq", {
        ...
        chisq.p <- rvgt.range.chisq(gen.data = mgt, 
...
test_that("[chisq-i03] calling rvgt.range.erc with invalid arguments: gen.data", {
        ## gen.data

        msg <- "Argument 'gen.data' is missing"
        expect_error( rvgt.range.chisq(
          rdist=rnorm, dist.params=dp, n=1000, breaks=10),  msg)

        msg <- "Argument 'gen.data' must be of class \"rvgt.range.time\"."
        expect_error( rvgt.range.chisq(
          gen.data=1, rdist=rnorm, dist.params=dp, n=1000, breaks=10),  msg)
...
test_that("[chisq-i04] calling rvgt.range.erc with invalid arguments: qdist, pdist", {
        ...
        expect_error( rvgt.range.chisq(
          gen.data=mgt, n=1000, breaks=10),  msg)

        msg <- "Argument 'pdist' invalid."
        expect_error( rvgt.range.chisq(
          gen.data=mgt, n=1000, breaks=10, pdist=1),  msg)

        msg <- "Argument 'qdist' invalid."
        expect_error( rvgt.range.chisq(
          gen.data=mgt, n=1000, breaks=10, qdist=1),  msg)
})
