...
emprical distribution and tests for uniformity can be applied for all
distributions.
...
\item 
  ...
  Moreover, the theoretical distribution of the frequences
...
\item 
  ...
  $x$-scale as in Variant 1. Then compute the expected probablities
  for each bin by means of the distribution function.
  This variant is as fast as Variant 1 but is less sensitive against
  round-off errors. On the other hand, the bin frequences are slightly
...
plot(ft)
@ 
  \caption{Histogram of a defectiv sample of normal distributed random
...
\subsection{Perturbate a Random Variate Generator}
...
pertubate existing random variate generators. Thus one can investigate
